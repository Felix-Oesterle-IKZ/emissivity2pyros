...
global istStationaer

# Funktionen:
###########################################################################
def Init_File():                                                          # Erstelle die Köpfe der File-Datei
###########################################################################
    global FileOutName, FileOutNameE, FileOutNameEEnd, AutoStop_Pt, AutoStop_Hp, AutoStop_Py, Folder, istStationaer, temppyro2List
...
#     )
    ...
    with open(Folder + '/' + FileOutName,"w", encoding="utf-8") as fo:
        ...
        for name, pt in pt_sam.items():
            fo.write(f"Temp. {name} [°C]".ljust(35))
        fo.write(f"Temp. {pyro2.name} [°C]".ljust(35))
    ...
    with open(Folder + '/' + FileOutNameE,"w", encoding="utf-8") as foE:
        ...
        for name, Pyro in pyroLW.items():
            foE.write(f"Emiss. {name} [%]".ljust(35))
        foE.write(f"Emissions-Messgerät".ljust(15))
        foE.write('\n')
    ...
    with open(Folder + '/' + FileOutNameEEnd,"w", encoding="utf-8") as foEE:
        ...
        for name, Pyro in pyroLW.items():
            foEE.write(f"Emiss. {name} [%]".ljust(35))
        foEE.write(f"Emissions-Messgerät".ljust(15))
        foEE.write('\n')
...
###########################################################################
    global loop, StartConfig, nextTempIn, nEMess, end, istStationaer, temppyro2List
    ...
    for name, pt in pt_sam.items():
        if pt.vergleich == True:
            tempVergleichPt = pt.list[-1]             # Der Letzte Wert in der ausgewählten Liste ist der aktuelle Wert!
            if tempVergleichPt == 0:                  # Fehlerbehandlung
                tempVergleichPt = pt.list[-2]
                if args.log == True:   logging.info('Vergleichstemperatur war Null!')
    ...
    with open(Folder + '/' + FileOutName,"a", encoding="utf-8") as fo:
        ...
        fo.write(f"{temppyro2List[-1]:<35.1f}")
        fo.write('\n')
    ...
    Stationaer = float(TempStationar[loop])
    StationarTime = int(TempStationarTime[loop]) * 30
    EmisMessGerät = EmisMessGerätListe[loop]
    ...
    if len(pt.list) > StationarTime: # Nur prüfen wenn über StationarTime Einträge vohanden sind
        tempList = list(pt.list[-StationarTime:]) # Liste der letzten StationarTime Temperaturen erstellen...
        tempList.sort() # ...und sorteiren
        #print(f"Größte   Temp:{round(tempList[-1],2)}\nKleinste Temp:{round(tempList[0],2)}")
        
        if tempList[-1] - tempList[0] <= Stationaer:# and jetzt >= StationaerLock : # Wenn die größte die Temperaur und die Kleinste Temp. kleiner sind als der vorggebene Bereich
            istStationaer = True
            #StationaerLock = datetime.datetime.now() + datetime.timedelta(minutes=Time)
            #print("Stationaer")
        else:
            istStationaer = False
            StationaerLock = False
            #print("Nicht Stationaer")
    
    
    #print(f"aktuelle Temp: {tempPt}")
    #print(f"istStationaer: {istStationaer}")
    #print(f"{jetzt}, {nextTempIn}")
    #print()
    
    # Wenn die Temperatur innhalb des Bereiches ist und die Temeratur stationär ist.
    if (Soll + Area) > float(listTempPt[-1]) and (Soll - Area) < float(listTempPt[-1])  and istStationaer == True:                                # Solange der Istwert außerhalb des Bereiches ist wird StartConfig auf False gesetzt
        ...
        if EmisMessGerät == "pyro":                                                                # das soll nur 16 mal durchgeführt werden, da die Rundung nah 10 - 13 Durschgängen keine Änderung mehr bringt
            ...
            if nEMess < 30:
                with open(Folder + '/' + FileOutNameE,"a", encoding="utf-8") as foE:        # Anpassung durchführen
                    ...
                    for name, Pyro in pyroKW.items():
                        Pyro.e_py, Pyro.e_Drauf = Emissions_Anpassung(Pyro.listT[-1], tempVergleichPyro, Pyro.e_py, Pyro.e_Drauf, 100, 5)
                        Pyro.write_pyro_para('e', Pyro.e_py)
                        foE.write(f'{Pyro.e_py:<35}')
                    
                    for name, Pyro in pyroLW.items():
                        Pyro.e_py, Pyro.e_Drauf = Emissions_Anpassung(Pyro.listT[-1], tempVergleichPyro, Pyro.e_py, Pyro.e_Drauf, 100, 10)
                        Pyro.write_pyro_para('e', Pyro.e_py)
                        foE.write(f'{Pyro.e_py:<35}')
                    foE.write("pyro".ljust(15))
                    foE.write('\n')
                    nEMess += 1
                    
            elif nEMess == 30:                                                                       # Speichere die endgültigen Emissionsgrade
                with open(Folder + '/' + FileOutNameEEnd,"a", encoding="utf-8") as foEE:
                    foEE.write(f'{Soll:<25}')
                    foEE.write(f'{tempVergleichPyro:<35.1f}')
                    for name, Pyro in pyroKW.items():
            ...
            if nEMess < 30:
                with open(Folder + '/' + FileOutNameE,"a", encoding="utf-8") as foE:        # Anpassung durchführen
                    foE.write(f'{time_abs:<15}{dt:<15.1f}')
                    for name, Pyro in pyroKW.items():
                        Pyro.e_py, Pyro.e_Drauf = Emissions_Anpassung(Pyro.listT[-1], tempVergleichPt, Pyro.e_py, Pyro.e_Drauf, 100, 5)
                        Pyro.write_pyro_para('e', Pyro.e_py)
                        foE.write(f'{Pyro.e_py:<35}')
                    
                    for name, Pyro in pyroLW.items():
                        Pyro.e_py, Pyro.e_Drauf = Emissions_Anpassung(Pyro.listT[-1], tempVergleichPt, Pyro.e_py, Pyro.e_Drauf, 100, 10)
                        Pyro.write_pyro_para('e', Pyro.e_py)
                        foE.write(f'{Pyro.e_py:<35}')
                    foE.write("pt".ljust(15))
                    foE.write('\n')
                    nEMess += 1
                ...
                with open(Folder + '/' + FileOutNameEEnd,"a", encoding="utf-8") as foEE:
                    foEE.write(f'{Soll:<25}')
                    foEE.write(f'{tempVergleichPt:<35.1f}')
                    for name, Pyro in pyroKW.items():
                        foEE.write(f'{Pyro.e_py:<35}')
                    for name, Pyro in pyroLW.items():
                        foEE.write(f'{Pyro.e_py:<35}')
            ...
            print("ERROR: EmisMessGerät muss 'pyro' oder 'pt' sein!")
                                                                                     # Der Emissionsgrad wird aus dem Gerät gelesen und in eine Liste getan
        ...
        if jetzt >= nextTempIn:                                                         # Vergleicht die beiden Zeiten
            ...
            if loop == (lenght-1):                                                      # bei erreichen des Endes wird das Programm beendet!
                ...
                if Stop_Graph == True:                                                                      # Wenn die aktuelle Grafik gesperrt ist, so wird die Grafik vor beenden neu erstellt
                    ...
                    pyro2.grafik_T(ax3End, ListTiRe)
    ...
    ### Wenn nicht im Bereich und/oder nicht stationär    
    else:
...
pyro2 = pyrometer_lumasense.PyrometerLumasense(config["IGAR-6-adv"], name="Pyro. (SW, Ratio)")
...
nextTempIn = str(datetime.datetime.now() + datetime.timedelta(hours=24)).split('.')[0] # Muss am Anfang groß sein um sofortiges ausführen der Messung zu verhindern
