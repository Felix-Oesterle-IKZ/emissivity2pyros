...
class PyrometerLumasense:
    ...
    def __init__(self, config, name="PyrometerLumasense"):
        ...
        if type(self.serial) != SerialMock:
            ...
            self.set_k(config["k"])
            self.set_operationMode(config["operationMode"])
    ...
    @property
    def k(self):
        """Read the current k value"""
        if type(self.serial) == SerialMock:
            return -1
        cmd = f"{self.device_id}ev\r"
        self.serial.write(cmd.encode())
        return self._get_float()
    
    def set_operationMode(self, operationMode):
        """Set OperationMode and check if it was accepted."""
        logger.info(f"{self.name} - setting OperationMode {operationMode}")
        cmd = f"{self.device_id}ka{operationMode}\r"
        self.serial.write(cmd.encode())
        self._get_ok()
        #assert self.operationMode == operationMode
    
    def set_k(self, k):
        """Set k and check if it was accepted."""
        logger.info(f"{self.name} - setting k {k}")
        cmd = f"{self.device_id}ev{k*100:05.1f}\r".replace(".", "")
        self.serial.write(cmd.encode())
        self._get_ok()
        assert self.k == k * 100
